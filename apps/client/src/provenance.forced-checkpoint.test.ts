/**
 * Forced-checkpoint node shape (protocol §8: "When a Step does seal, it mints a
 * node carrying the snapshot").
 *
 * A deliberate Step/Send seals a new node even when content is unchanged since
 * the last seal. Such a node carries `deltas: []` (nothing changed) plus the
 ...
 *  kind 4290, `z:file`, JSON content with `sealedAt` + `deltas` + `snapshot`. */
function fileNode(
  id: string,
  prev: string | null,
  content: { sealedAt: number; deltas: unknown[]; snapshot?: string },
): Event {
  return {
    id,
    pubkey: "pk",
...
test("forced checkpoint: two in a row stay content-stable and linear", () => {
  // Edit seals n1 with content "hello". Then two forced Steps (n2, n3) seal
  ...
  const n2 = fileNode("n2", "n1", { sealedAt: 2000, deltas: [], snapshot: "hello" });
  const n3 = fileNode("n3", "n2", { sealedAt: 3000, deltas: [], snapshot: "hello" });
...
test("forced checkpoint after an edit still reconstructs (snapshot carries through)", () => {
  ...
  const n3 = fileNode("n3", "n2", { sealedAt: 3000, deltas: [], snapshot: "hello world" });
...
test("forced checkpoint: deltas:[] with no snapshot falls back to prior content", () => {
  // A malformed/legacy forced node that shed its snapshot but kept deltas:[]
  ...
  const n2 = fileNode("n2", "n1", { sealedAt: 2000, deltas: [] });
  assert.equal(reconstructFromChain([n1, n2]), "hello");
});
