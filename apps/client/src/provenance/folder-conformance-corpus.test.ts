...
function chain(vector: { nodeIds: string[] }): Event[] {
  return vector.nodeIds.map((nodeId) => {
    ...
    assert.equal(event.id, nodeId, `desktop corpus key does not match signed id ${nodeId}`);
    return event;
  });
}
...
test("desktop enforces exact recursive operation membership and propagation", () => {
