...
const MAX_ATTESTATION_TARGETS_PER_QUERY = 32;
...
function ordinaryCitationTargets(event: Event): string[] {
  return [...new Set(eventMeta(event).citationTargets)].filter((id) => HEX_64.test(id));
}
...
function isSocialCarryingEvent(event: Event): boolean {
  return (
    ...
    // q-tags on an LLM node include prompt-scope dependencies. Until the wire
    // distinguishes those from social citation roles, do not leak that scope
    // into global rendezvous.
    !event.tags.some((tag) => tag[0] === "scope" && tag[1] === "llm")
  );
}

/** Validate the immutable Coin genesis before checking compound-Mint completion. */
async function verifiedCoinGenesisCoordinate(event: Event): Promise<string | null> {
...
function isValidMinterAttestation(coin: Event, attestation: Event): boolean {
  if (
...
async function fetchMinterAttestations(
  ...
  for (const targetBatch of batches(targetNodeIds, MAX_ATTESTATION_TARGETS_PER_QUERY)) {
    throwIfAborted(signal);
    ...
    const targets = new Set(targetBatch);
    for (const hit of result.hits) {
      const target = hit.event.tags.find(
        (tag) => tag[0] === "e" && tag[3] === "target" && targets.has(tag[1] ?? ""),
      )?.[1];
      if (!target) continue;
  ...
  try {
    ...
    async function verifyNextRelays(): Promise<void> {
      while (nextRelay < relayGroups.length) {
        ...
        const ownerByTarget = new Map<string, RendezvousPointer[]>();
        for (const pointer of pointers) {
          ...
          for (const targetNodeId of ordinaryCitationTargets(carrying)) {
            const owners = ownerByTarget.get(targetNodeId) ?? [];
            owners.push(pointer);
            ownerByTarget.set(targetNodeId, owners);
          }
        }

        const matched = new Set<string>();
        for (const targetBatch of batches([...ownerByTarget.keys()], MAX_RELAY_BATCH_IDS)) {
          throwIfAborted(deadline.signal);
          ...
          for (const [targetNodeId, target] of coinGenesis) {
            ...
            for (const pointer of ownerByTarget.get(targetNodeId) ?? []) {
              const key = `${pointer.relayUrl}\n${pointer.eventId}`;
          ...
          if (matched.size === pointers.length) break;
        }
      }
    }
    await Promise.all(
