  ...
  server.tool(
    ...
    async ({ relativePath, content, tags, replyingTo, citationIds }) => runMcpMintLifecycle(async () => {
      ...
      const nodeId = await workspace.writeFile(
        ...
        // one atomic agent EditorTransaction from the previous snapshot to this one.
        undefined,
        true, // localOnly — the sovereignty filter
        true, // an explicit Step always mints one checkpoint
        undefined, // operation id — generated by the workspace
        undefined, // ordinary MCP Step has no external publication fence
        null, // synthesize one current-dialect EditorTransaction after recovery
      );
      return jsonResult({
        nodeId,
        folderId: ref.id,
  ...
  server.tool(
    ...
    async ({ relativePath, content, tags, replyingTo, citationIds }) => runMcpMintLifecycle(async () => {
      ...
      const nodeId = await workspace.writeFile(
        ...
        // EditorTransaction; unchanged Send reuses the latest Step below.
        undefined,
        true, // Record pending changes locally before distribution.
        false, // Unchanged Send reuses the latest Step.
        undefined, // operation id — generated by the workspace
        undefined, // ordinary MCP Send has no external publication fence
        null, // synthesize one current-dialect EditorTransaction after recovery
      );
      const event = await fetchEventById(nodeId);
