...
- `role: "content"` — a quote of **orphan text** with no origin node in the system (print, oral, sourceless). The rendezvous cite: keyed on `hash` (the content hash `H = sha256(canonical(newValue))`), not on `sourceEventId`. Carries the verbatim bytes in `newValue` (verification + content), an OPTIONAL `source` (work/edition/locator — distinguishes readers from scrapers), and a `relayHint` for co-citer reachability. Derives a top-level `["cite-content", H, relayHint, "implicit"|"attested"]` tag — see `rendezvous.md` §1. The only cite role without a `sourceEventId` or matching `q`: there is no node to dereference.
...
22. **Anteriority moved from Affirm to Step; `role: "content"` cite + rendezvous layer added.** Three coupled changes introduced by the rendezvous companion doc (`rendezvous.md`), together closing the "how do two people who quoted the same text find each other, and how does the recipient know the other is real" gap that the provenance + transport pair deliberately left open.
  ...
  (b) **`role: "content"` cite (extends §3.3).** A fifth cite role for quoting **orphan text** with no origin node in the system (print, oral, sourceless). Keyed on `hash` (`H = sha256(canonical(newValue))`), not `sourceEventId`; carries the verbatim bytes (verification + content) and an OPTIONAL `source` (work/edition/locator). Derives a top-level `["cite-content", H, relayHint, "implicit"|"attested"]` tag — the rendezvous coordinate. The only cite role without a matching `q`: there is no node to dereference. See `rendezvous.md` §1.
...
- **Kind and tag registration.** `4290`/`34290`/`34291` and the `z`/`f`/`F` letters are placeholders pending collision review. `34292` is *held* for `TraceAnnotation` (§11) — sketched but not yet specified to implementation depth; the number is reserved, not burned. The `cite-content` top-level tag (§3.3 `role: "content"`) is a new tag name pending the same collision review.
