...
```json
{ "type": "insert | delete | replace", "position": { "start": 42, "end": 42 }, "newValue": "…", "timestamp": 1730000000000 }
```
...
`affirm > merge > fork > delete > llm > import > cite > paste > edit > focus`
...
`authors` is an ordered run list covering `snapshot`: run *k* attributes the slice beginning at the sum of all prior `len` values, for `len` UTF-16 code units, to pubkey `v`. The `len` values MUST sum to exactly the snapshot's length; on any mismatch a reader MUST treat `authors` as absent and fall back to node-signer attribution. Runs carry no text — the body is stored once, in `snapshot`.
...
**Verification rule.** All three attribution signals on a node — signer, `authors`, `contributors` — are claims by the node's signer. A run attributed to pubkey P is **verified** when its text is derivable from a node signed by P reachable from this node via the seam edges: `merge-parent`, `extracted-from`, `forked-from`, or `q`. `src` names the corroborating node directly; without it a verifier searches the seam. A run that cannot be corroborated is **asserted**, and clients SHOULD render the two states distinguishably. This gives attribution the same epistemic status as citation (§3.9): asserted, cheaply checkable, degradable — never silently trusted, never requiring the attributed author's cooperation to seal.

**Multi-author means multi-chain.** Concurrent authorship happens under distinct keys on distinct chains, joined by `merge` — which is what makes every legitimate cross-author run corroborable in principle (the merge-parent *is* a node the other author signed). A second human typing inside one session, under one key, has no chain and stays permanently asserted; that mode is out of scope for verified attribution.
...
- **External write (file).** File changed outside the traced editor; deltas computed by diffing last-known content against disk.
...
**Per-delta events were rejected** because signatures at delta granularity within one author add nothing (same signer) — the marginal value exists only at cross-author seams, which are exactly the edges already signed. The global costs are real: per-keystroke event volume abandons bounded-frequency sealing (§R1), a signed keystroke stream turns a private writing rhythm into a publishable permanent artifact by default, and the LLM/agent case is harness-signed either way. Paying a global cost for seam-local value the graph already provides is the wrong trade.

What remains genuinely uncoverable: two humans under one key in one session — no chain, no seam, permanently asserted. Multi-author means multi-chain, joined by merge; the spec says so rather than pretending otherwise.
...
13. **Per-delta signing and co-signed `authors` — considered and rejected**; see §R5.
...
- **`affirm` wire mechanics.** Affirm is now specified as a concept (§8, §3.4, §R11.19): a post-Send publication act, decoupled from ZINE. What remains open is the wire encoding — whether `action: "affirm"` lives on a new sealed node citing the sent node (analogous to `forked-from`), or is a parameterized-replaceable event keyed to the sent node's id (analogous to `TraceOpinion`). The former is a chain event; the latter is a side-channel pointer. Either preserves the "affirm requires prior send" invariant; the choice is about whether affirmation is history (chain) or current-state (replaceable).
